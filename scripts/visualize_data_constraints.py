...
for file in state_files:
    ...
    zero_obs = np.zeros((1, 300, obs_dim), dtype=np.float32)
    zero_action = np.zeros((1, 300, action_dim), dtype=np.float32)
...
ax.plot([ax_limits[0][0], ax_limits[0][1]], [0.25, 0.25], color=[0.4, 1, 0.4], linewidth=5)

# ax.set_title('Trajectories in the X-Y Plane')
ax.set_facecolor([1, 1, 0.9])
ax.set_xlim(ax_limits[0])
ax.set_ylim(ax_limits[1])
fig.savefig('logs/figures/avoiding_data.png', bbox_inches='tight')
plt.savefig('logs/figures/avoiding_data.pdf', bbox_inches='tight', format='pdf')
...
for j, halfspace_variant in enumerate(halfspace_variants):
    ...
    ax.set_xlim(ax_limits[0])
    ax.set_ylim(ax_limits[1])
fig.savefig('logs/figures/avoiding_constraints.png', bbox_inches='tight')
fig.savefig('logs/figures/avoiding_constraints.pdf', bbox_inches='tight', format='pdf')
...
fig.savefig('logs/figures/avoiding_velocities.png', bbox_inches='tight')
...
fig.savefig('logs/figures/avoiding_episode_lengths.png', bbox_inches='tight')
plt.show()
