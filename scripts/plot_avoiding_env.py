...
for i, ax in enumerate(axes):
    ax.set_title(f"{halfspace_variants[i]}", fontsize=16)
    if halfspace_variants[i] == 'top-left-hard':
        polytopic_constraints = [config['halfspace_constraints'][exp][0]]
        obstacle_constraints = [config['obstacle_constraints'][exp][0]]
    elif halfspace_variants[i] == 'top-right-hard':
    ...
    # Add very thick light green line at y = 0.30
