...
def _is_hard_conditioning(cond):
    return isinstance(cond, dict) and all(isinstance(k, int) for k in cond.keys())
...
class GaussianDiffusion(nn.Module):
    ...
    #------------------------------------------ denoising during sampling/Inference----------------#
    @torch.no_grad()
    def p_sample(self, x, cond, t, returns=None, projector=None, constraints=None):
        '''
            one reverse diffusion step
            It implements the transition:            xt→xt-1
            So its “one denoising step.”
        '''
    ...
    def p_sample_loop(self, shape, cond, returns=None, return_diffusion=False, projector=None, constraints=None, repeat_last=0):
        '''
            ...
            full reverse diffusion step
            It repeatedly calls p_sample(...) from t = T-1 ... 0:
            x_T → x_T-1 → ... → x_0
    ...
    def conditional_sample(self, cond, returns=None, horizon=None, *args, **kwargs):
        ...
        if isinstance(cond, dict):
            # existing behavior: dict of {t: obs}
            any_val = next(iter(cond.values()))
            batch_size = any_val.shape[0]
        elif torch.is_tensor(cond): 
            batch_size = cond.shape[0] # new behavior: global conditioning vector
        else: 
    ...
    def q_sample(self, x_start, t, noise=None):
        """
        Forward diffusion step(adding noise)
        """
        if noise is None:
