...
def formulate_halfspace_constraints(constraint, enlarge_constraints, trajectory_dim, act_obs_indices):
    ...
    # # -------------------------
    # # NEW: 3D plane halfspace
    # # n·p <= d
    # # constraint = {type: 'plane', dimensions: ['x','y','z'], normal: [nx,ny,nz], d: float}
    # # -------------------------
    # if isinstance(constraint, dict) and constraint.get("type", None) == "plane":
    #     dims = constraint.get("dimensions", constraint.get("dims", None))
    #     if dims is None:
    #         raise ValueError("Plane halfspace needs 'dimensions' (e.g. ['x','y','z']).")
    #     n = np.asarray(constraint["normal"], dtype=np.float32)
    #     d = float(constraint["d"])
    #     # tighten constraint by shifting plane inward along its normal
    #     # For n·p <= d, decreasing d shrinks the feasible region.
    #     if enlarge_constraints != 0.0:
    #         d = d - enlarge_constraints * np.linalg.norm(n)
    #     C_row = np.zeros(trajectory_dim, dtype=np.float32)
    #     for i, dim in enumerate(dims):
    #         if dim not in act_obs_indices:
    #             raise KeyError(f"Dimension '{dim}' not found in act_obs_indices.")
    #         C_row[act_obs_indices[dim]] = n[i]
    #     return C_row, d
    
    # -------------------------
    # OLD: 2D line halfspace in XY
    # -------------------------
...
def formulate_dynamics_constraints(exp, act_obs_indices, action_dim):
    ...
    if 'avoiding' in exp and action_dim > 0:
        dynamic_constraints = [
            ...
            ('deriv', np.array([act_obs_indices['x'], act_obs_indices['vx']])),
            ('deriv', np.array([act_obs_indices['y'], act_obs_indices['vy']])),
            # ('deriv', np.array([act_obs_indices['x'], 0])),
            # ('deriv', np.array([act_obs_indices['y'], 1])),
        ]
...
def plot_environment_constraints(exp, ax, flip_xy=False):
    ...
    if exp == 'pointmaze-medium-dense-v2':
        ...
        centers = [[0.2, 0.0], [0.4, -0.1], [0.4, 0.1], [0.6, -0.2], [0.6, 0.0], [0.6, 0.2]]
        ...
        # ax.plot([0.2, 0.8], [0.25, 0.25], color=[0.4, 1, 0.4], linewidth=5)
    elif exp == 'avoiding-d3il-v1':
        # centers = [[0.2, 0.0], [0.4, -0.1], [0.4, 0.1], [0.6, -0.2], [0.6, 0.0], [0.6, 0.2]]
        centers = [[0.284, 0.144], [0.463, 0.15], [0.615, 0.146], [0.353, 0.018], [0.553, 0.009], [0.456, -0.114]]
        for center in centers:
            if flip_xy:
                ax.add_patch(matplotlib.patches.Circle(center[::-1], 0.025, color='r'))
            ax.add_patch(matplotlib.patches.Circle(center, 0.025, color='r'))
...
def plot_halfspace_constraints(exp, polytopic_constraints, ax, ax_limits, flip_xy=False, enlarge_constraints=0):
    ax.plot([1.0, 1.0], [ax_limits[1][0], ax_limits[1][1]],  color=[0.4, 1, 0.4], linewidth=5)
