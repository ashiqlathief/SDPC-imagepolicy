...
parser = argparse.ArgumentParser(description="Fly Crazyflie on a figure-8 path.")
...
data_dir = project_path("isaac", "dataset", "figure8_crazyflie", "data0")
...
# ─────────────────────────────────────────────────────────────────────────────
# Zarr dataset writer  (self-contained copy — avoids importing quadcopter.py
# which creates its own AppLauncher at module level)
# ─────────────────────────────────────────────────────────────────────────────
...
def save_dataset(episodes_states, episodes_images, curr_states, curr_images,
    ...
    if len(curr_states) > 0:
        episodes_states.append(np.stack(curr_states, axis=0))   # (T, N, state_dim)
        episodes_images.append(np.stack(curr_images, axis=0))   # (T, N, H, W, 3)
        curr_states.clear()
        curr_images.clear()
...
# ─────────────────────────────────────────────────────────────────────────────
# Figure-8 path  (obstacle-aware waypoints)
#
# The path flies at z = 0.5 m — inside the obstacle zone (z ∈ [0, 1] m).
# It loops at y = ±0.65 to stay clear of the obstacle cluster (y ∈ [-0.4, 0.4]).
# Near x = 0.5 it pinches to y = ±0.45 to clear the side boxes at (0.5, ±0.7).
#
# Shape viewed from above (x = right, y = up):
#
#   y=+0.65  ----upper loop ( x: 0.5 → 4.2 )----
#   y= 0.00  ×(x=0.2)                          ×(x=4.2)   crossover points
#   y=-0.65  ----lower loop ( x: 4.2 → 0.5 )----
#
# Obstacle clearances (minimum):
#   boxes at y=±0.7     → fly at y=±0.45 near x=0.5   → 0.15 m gap
#   cylinders at y=0.4  → fly at y=+0.65               → 0.19 m gap
#   boxes/cyls at y=0.2 → fly at y=+0.65               → 0.35 m gap
# ─────────────────────────────────────────────────────────────────────────────

def make_obstacle_avoiding_path(n=40, z=0.5):
    """
    Return (n, 3) waypoints for one full figure-8 lap at flight altitude z.

    The skeleton below defines the key turning points; the function then
    interpolates them to n equidistant waypoints so the waypoint density
    is uniform around the loop.

    To use with a DIFFERENT obstacle layout, change the skeleton coordinates
    so the loops bypass whatever obstacles you have placed.
    """
    # Key skeleton points (x, y) — z is added uniformly below.
    # Adjust these to change how the path threads through obstacles.
    skeleton = np.array([
        (0.2,  0.00),   # crossover at corridor start (drone spawn area, clear)
        (0.5,  0.45),   # pinch: clear box(0.5, 0.7)   → 0.15 m gap from edge
        (1.5,  0.65),   # upper loop: clear of all obstacles at y=0.65
        (2.5,  0.65),
        (3.5,  0.65),
        (4.2,  0.00),   # crossover at corridor far end (past all obstacles)
        (3.5, -0.65),   # lower loop
        (2.5, -0.65),
        (1.5, -0.65),
        (0.5, -0.45),   # pinch: clear box(0.5, -0.7)  → 0.15 m gap from edge
        (0.2,  0.00),   # close the loop back at start
    ], dtype=np.float32)

    # Interpolate skeleton to n equidistant points along arc length
    diffs    = np.diff(skeleton, axis=0)
    seg_lens = np.hypot(diffs[:, 0], diffs[:, 1])
    cumlen   = np.concatenate([[0.0], np.cumsum(seg_lens)])
    s_query  = np.linspace(0.0, cumlen[-1], n, endpoint=False)

    xs = np.interp(s_query, cumlen, skeleton[:, 0])
    ys = np.interp(s_query, cumlen, skeleton[:, 1])
...
def main():
    ...
    sim.set_camera_view(eye=[4.5, -3.5, 3.0], target=[2.5, 0.0, 1.2])
    ...
    # ── figure-8 waypoints ────────────────────────────────────────────────────
    one_lap   = make_obstacle_avoiding_path(args_cli.n_waypoints)
    ...
    nxt_xy      = np.roll(all_wps[:, :2], -1, axis=0)
    ...
    print("[INFO] Obstacle-avoiding figure-8 navigation")
    print(f"       Waypoints per lap : {args_cli.n_waypoints}")
    print(f"       Laps              : {args_cli.laps}  →  {total_wps} targets total")
    print(f"       Flight altitude   : z=0.5 m  (inside obstacle zone, clear of all obstacles)")
    print(f"       Path extents      : x∈[0.2,4.2]  y∈[-0.65,+0.65]")
    print(f"       Acceptance radius : {args_cli.wp_radius} m")
    print()

    # ── flight state ──────────────────────────────────────────────────────────
    HOVER_TARGET = torch.tensor([[0.0, 0.0, 0.5]], device=device).expand(num_envs, -1)
    ...
    while simulation_app.is_running():
        ...
        if reset:
            ...
            # restart figure-8 from the hover phase
            wp_idx      = -1
            hover_count = 0
            reset_pid()
        ...
        # ── phase 2: follow figure-8 waypoints ───────────────────────────────
        else:
    ...
    out_path  = os.path.join(args_cli.save_dir, f"figure8_{timestamp}.npz")
    np.savez(
        out_path,
